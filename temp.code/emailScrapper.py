...
class EmailScrapper:
    ...
    def __init__(self, query, DEBUG=False):
        ...
        if DEBUG:
            oprions.add_argument("--headless")
            oprions.add_argument("--no-sandbox")
            oprions.add_argument("--disable-dev-shm-usage")

        oprions.add_argument("--disable-search-engine-choice-screen")
        oprions.add_experimental_option("prefs", {"profile.default_content_settings.cookies": 2}) 
        self.driver = webdriver.Chrome(options=oprions)
        self.driver.get(f'https://www.google.com/search?q={query}')
        try:
            # Clicking Ok on Cookies Acceptance of Google Search Engine
            self.driver.find_element(By.XPATH, '//*[@id="W0wltc"]').click() # Atleast for Germany
        except:
            pass
        time.sleep(1)
        self._find_emails()
